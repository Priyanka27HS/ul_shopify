export class SwipeUtil {
    ...
    async swipeUpByPercentage(startPercentage: number = 80, endPercentage: number = 20): Promise<void> {
        const screenSize = await driver.getWindowRect();
        const x: number = screenSize.width * (50 / 100);
        const startY: number = screenSize.height * (endPercentage / 100);
        const endY: number = screenSize.height * (startPercentage / 100);

        await this.swipe(x, x, startY, endY);
    }

    async horizontalSwipe(startX: number = 200, endX: number = 800, y: number = 500): Promise<void> {
